...
//
const injectedCssClasses = '.target-to-source-language-wrapper,.target-to-source-language,.target-to-source-language-tooltip-text'
	...
	if (verifiedMatches && verifiedMatches.length > 0) {
		...
		for (const match of verifiedMatches) {
			...
			if (specificSourceLanguageToTargetLanguage) {
				...
				// TODO change 2
				let replacement = Math.random() < 2 ? randomTargetLanguageWord : matchText
				...
				const html = `<span class='target-to-source-language-wrapper' style=\"${unsetEverything}\">` +
						`<abbr class="target-to-source-language-tooltip-text" title="${matchText}">` +
						`<span class='target-to-source-language'>` +
							`${replacement}` +
							`</span>` +
						`</abbr>` +
					`</span>`
