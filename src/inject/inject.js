...
const buildKnownSourceLanguageWordsSelector = () => {
	const sourceLanguageCssQueries = sourceLanguageToTargetLanguageEntries.map(([ sourceLanguage ]) => {
...
//
const injectedCssClasses = '.target-to-source-language-wrapper,.target-to-source-language,.target-to-source-language-tooltip-text,.duo-replaced,.duo-skipped'
...
const getInnerMostSourceLanguageElements = (containerSelector) => {
	...
	const allSourceLanguageMatchedElements = $(containerSelector).find(knownSourceLanguageWordsSelector)
	const innermostSourceLanguageElements = allSourceLanguageMatchedElements.not(allSourceLanguageMatchedElements.has(allSourceLanguageMatchedElements))
	...
	const innermostWithoutMarked = innermostSourceLanguageElements.not(injectedCssClasses).not(innermostSourceLanguageElements.has(injectedCssClasses))
...
const createIndividuaSourceLanguageRegexString = (sourceLanguage, targetLanguageWords) =>
	// select sourceLanguage phrases, match at word breaks
	`(\\b${sourceLanguage}\\b)`

// match a single sourceLanguage phrase with regex
const createIndividualSourceLanguageRegex = (sourceLanguage, targetLanguageWords, flags = 'gi') => {
	const individualSourceLanguageRegex = createIndividuaSourceLanguageRegexString(sourceLanguage, targetLanguageWords)
...
const buildAllSourceLanguagePhrasesRegex = () => {
	...
	const sourceLanguagePhraseRegexStr = sourceLanguageToTargetLanguageEntries
		.map(([sourceLanguage, targetLanguageWords]) => createIndividuaSourceLanguageRegexString(sourceLanguage, targetLanguageWords)) // (surely is not)
...
const findSpecificSourceLanguagePhrase = (text) => {
	...
	const targetLanguagePhrases = sourceLanguageToTargetLanguageDuolingo[normalizedText]
	console.log('normalizedText', normalizedText, 'targetLanguagePhrases', targetLanguagePhrases)

	// search for the specific possible sourceLanguage phrase we matched, so we can verify they are an sourceLanguage phrase
	const specificSourceLanguageToTargetLanguage = sourceLanguageToTargetLanguageEntries.find(([sourceLanguagePhrase, targetLanguageWords]) => {
		individualSourceLanguagePhraseRegexAll = createIndividualSourceLanguageRegex(sourceLanguagePhrase, targetLanguageWords)
		individualSourceLanguagePhraseRegexFirst = createIndividualSourceLanguageRegex(sourceLanguagePhrase, targetLanguageWords, 'i')
...
const replaceWords = (innerMostNode) => {
	...
	const verifiedMatches = getMatchesInArray(allSourceLanguagePhrasesRegex, text).reverse()
	...
	if (verifiedMatches && verifiedMatches.length > 0) {
		...
		for (const match of verifiedMatches) {
			...
			const { specificSourceLanguageToTargetLanguage, specificSourceLanguageToTargetLanguageRegexAll, specificSourceLanguageToTargetLanguageRegexFirst } = findSpecificSourceLanguagePhrase(matchText)
			console.log('specific phrase (possibly, might be buggy)', specificSourceLanguageToTargetLanguage)



			if (specificSourceLanguageToTargetLanguage) {

				const specificSourceLanguageToTargetLanguageRegexFirst = createIndividualSourceLanguageRegex(matchText, null, 'i')
				console.log('About to replace match', matchText)
				const [sourceLanguagePhrase, targetLanguageWords] = specificSourceLanguageToTargetLanguage
				// console.log('about to update text', text)

				const randomIndex = Math.floor(Math.random() * targetLanguageWords.length)
				const randomTargetLanguageWord = targetLanguageWords[randomIndex]

				// If less than 0.2, replace with targetLanguage word
				// TODO change 2
				let shouldReplace = Math.random() < 0.5
				...
				const titleText = shouldReplace ? matchText + ` (${targetLanguagesAllDisplay})` : targetLanguagesAllDisplay

				numReplacements++
				const html = `<span class='target-to-source-language-wrapper' style=\"${unsetEverything}\">` +
						`<abbr class="target-to-source-language-tooltip-text" title="${titleText}">` +
						`<span class='${shouldReplace ? 'duo-replaced' : 'duo-skipped'}'>` +
							`${replacement}` +
							`</span>` +
						`</abbr>` +
					`</span>`
...
function restore_options() {
  ...
  chrome.storage.sync.get({
    ...
    console.log({ username, sourceLanguage, targetLanguage })
		...
		$.ajax({
			url: 'https://duolingo-django-api.herokuapp.com/source_to_target_phrases/',
			method: 'POST',
			data: {
			...
			.then(responseData => console.log('source to target phrases from api', responseData))
			.catch(error => console.error('failed to fetch source to target phrases', error))

		getInnerMostSourceLanguageElements('body')
			.each(function() {
				// if we've replaced enough on the page, stop
			if (numReplacements >= maxReplacements) {
				return
